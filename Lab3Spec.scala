...
class Lab3Spec(lab3: Lab3Like) extends FlatSpec {
  ...
  "InequalityVal" should "perform inequality evaluations" in {
    val v1 = B(true)
    val v2 = B(false)
    val v3 = N(4)
    val v4 = S("str")
    val v5 = S("11")
    assert(inequalityVal(Lt, v1, v2) === false)
    assert(inequalityVal(Le, v5, v4) === true)
    assert(inequalityVal(Ge, v3, v5) === false)
    assert(inequalityVal(Gt, v1, v2) === true)
